...
namespace MeshVolume
{
    public class DrawSphere
    {
        ...
        private float yc = 0;
        private float zc = 0;
        public DrawSphere(Control fm1)
        {
            form1 = fm1;
        }
        public DrawSphere(Control fm1, float r1, float xc1,
        float yc1, float zc1)
        {
            form1 = fm1;
            r = r1;
        ...
        public Point3[,] SphereCoordinates()
        {
            ...
            for (int i = 0; i < pts.GetLength(0); i++)
            {
                ...
                {
                    pts[i, j] = m.Spherical(r,
                    i * 180 / (pts.GetLength(0) - 1),
        ...
        public void DrawIsometricView(Graphics g)
        {
            ...
            PointF[,] pta = new PointF[pts.GetLength(0),
            pts.GetLength(1)];
            for (int i = 0; i < pts.GetLength(0); i++)
            {
                for (int j = 0; j < pts.GetLength(1); j++)
                {
                    pts[i, j].Transform(m);
                    pta[i, j] = Point2D(new PointF(pts[i, j].X,
                    pts[i, j].Y));
                }
            }
        ...
        private PointF Point2D(PointF pt)
        {
            ...
            aPoint.X = form1.Width / 2 + pt.X;
            aPoint.Y = form1.Height / 2 - pt.Y;
            return aPoint;
        }
